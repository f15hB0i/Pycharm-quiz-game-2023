...
class MotorcycleQuiz:
    def __init__(self, master):
        ...
        self.export_button = tk.Button(self.master, text="Export History", command=self.export_question_history, font=("Arial", 10))
    ...
    def export_question_history(self):
        if not self.question_history:
            messagebox.showinfo("Export History", "No question history to export.")
            return
            ...
            try:
                with open(file_path, "w") as file:
                    ...
                    for i, entry in enumerate(self.question_history):
                        ...
                        file.write(f"Your Answer: {entry['selected_option']}\n")
                        file.write(f"Correct Answer: {entry['correct_option']}\n\n")

                messagebox.showinfo("Export History", "Question history exported successfully.")
            except Exception as e:
                messagebox.showerror("Export History", f"Error exporting question history: {str(e)}")
        else:
            messagebox.showinfo("Export History", "Export canceled.")
